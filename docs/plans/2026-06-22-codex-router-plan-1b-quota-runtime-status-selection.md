...
- [ ] Two-process refresh test proves owner/follower behavior and stale-owner
      recovery against one router root.
...
| [ ] | 1B-16a | Old/missing/bad local HTTP token rejects before selection | spec Local Auth | T9/T12 | T9 / local auth | integration | rotated local token | `cargo test -p codex-router-core tests::local_auth_rejects_missing_empty_wrong_and_old_tokens -- --exact --list` | `cargo nextest run -p codex-router-core -- tests::local_auth_rejects_missing_empty_wrong_and_old_tokens --exact` | missing, empty, wrong, and old tokens reject before account selection | existing exact test listed once; local auth gate is exercised before selector | yes |
...
| [ ] | 1B-17a | Route classifier supports required routes and rejects Realtime | spec Supported Codex Traffic | T9 | T9 / route classifier | unit/protocol | route fixtures | `cargo test -p codex-router-proxy tests::route_classifier_supports_required_codex_routes_and_rejects_realtime -- --exact --list` | `cargo nextest run -p codex-router-proxy -- tests::route_classifier_supports_required_codex_routes_and_rejects_realtime --exact` | required route kinds classify; Realtime/WebRTC fails closed before selection | existing exact test listed once; unsupported paths fail before selection | yes |
...
| [ ] | 1B-25 | Installed smoke all-scenario wrapper is explicit | spec Smoke | T12 | T12 / installed Codex mock | smoke | installed Codex mock | `rg -n "EXPECTED_SCENARIOS|installed_codex::tests::installed_codex_" tests/smoke/installed_codex_mock.sh` | `tests/smoke/installed_codex_mock.sh` | wrapper enumerates expected scenarios explicitly, runs each once, prints count, and fails if any expected scenario is missing | source search lists both exact smoke scenarios and no broad prefix-only dispatch remains | yes |
...
- [ ] Matrix rows 1B-23 through 1B-26 pass or are explicitly `not-run` with
...
- [ ] Supported-route protocol/header proof passes for models `ETag`,
      WebSocket `x-models-etag` when implemented, memories trace summarize,
      responses compact, and unsupported Realtime fail-closed behavior.
