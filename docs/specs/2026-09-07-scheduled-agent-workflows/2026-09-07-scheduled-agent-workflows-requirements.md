...
| S16 | Agent, operator | Repeated runs continue the schedule-owned thread by default. An explicit fresh-thread-per-run option carries the prior Luna summary; each assigned execution thread remains exclusive to that schedule. Creating a schedule from an existing conversation forks it once, records the source as provenance, and uses the fork as execution destination. Subsequent source edits do not implicitly change that fork. |
