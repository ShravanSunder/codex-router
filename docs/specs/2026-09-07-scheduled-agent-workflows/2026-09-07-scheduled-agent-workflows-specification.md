...
Cancellation/pausing results return typed per-delivery evidence in dispatchedDeliveries. A cancelled wake-up can therefore report an accepted native receipt, a dispatch still in progress or an uncertain attempt without claiming recall. The result snapshot is taken at the cancellation transaction's ordering point; later changes remain inspectable.
