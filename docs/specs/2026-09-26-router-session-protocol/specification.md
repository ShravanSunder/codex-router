...
- **R23** Provider edge extensions (layer 3) MUST be translated into layer 1 or layer 2 at the Back door; a front door MUST never receive a provider-specific method name. Examples: Claude `_auth/status_update` becomes an `authenticationRequired` state; Cursor `cursor/create_plan` and `cursor/update_todos` become a `plan` Item.
