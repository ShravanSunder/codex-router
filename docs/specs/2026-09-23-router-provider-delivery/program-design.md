...
- Summaries: summary admission accepts any variant whose settlement is confirmed. Summary provenance uses one `SummarySourceReference` (`nativeTurn { turnId }` | `providerOperation { attemptId }`), owned by agent-automation and used by the protocol. It appears in the summary attempt, the stored completed summary source, the continuity input for the next run, and the public retained summary. Old stored `sourceTurnId` JSON decodes as `nativeTurn`, and the public run snapshot carries `sourceReference` only (hard cutover). No turn ID is ever fabricated for a provider run.
