...
- **R26** A scheduled run on a Claude or Cursor session MUST wait while that session is running (never steer it), start when idle, complete and produce its run summary from the provider's own prompt settlement, and on timeout request cancellation and finalize only when the provider operation settles; it MUST NOT read provider transcripts. If the provider's result text is no longer available when the summary is due (not retained, or the Host restarted after the operation settled), the run MUST keep its completed worker outcome with its required summary blocked and that cause shown, exactly as a native run whose summary source is missing, until an explicit summary skip or a successful recovery. (E1, E5)
...
- **R16** If the target E1 is a Claude or Cursor session not live elsewhere and not loaded in the running provider, then Router MUST load it and then deliver; if loading fails, the delivery MUST be `notSubmitted` with the load failure reason, and Router MUST NOT create a replacement session. (E1, E5)
- **R27** Router MUST locate a `claudeCodePeer` session only through Claude Code's local session registry by session ID. If the registry entry is missing, its process is gone, or its peer protocol version is not one Router supports, the session MUST NOT be treated as `claudeCodePeer`-reachable (fail closed). (E1, E7)
...
| R27–R30, C5 | Automated behaviour with a fake registry directory and a fake peer socket (live, gone, unknown protocol, busy/idle); manual runtime evidence: a Codex agent messages the owner's live Claude Code session through Router and the message arrives mid-turn, and Claude replies via `message_send`. |
