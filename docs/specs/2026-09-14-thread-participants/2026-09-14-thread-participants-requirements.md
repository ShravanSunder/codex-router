...
| U3 | Join is explicit. Posting, watching, listening, reading, and unresolving never create or reopen a Participant. A session identity cannot bypass `thread create` by posting with topic placement. | Must | Participants §2 invariants 2, 4, 7; Fable final review 2026-09-14 |
| U4 | An agent `thread create` states a Role and creates the root plus its Participant atomically. A session topic-placement post is refused with the exact `thread create` `nextAction`. A human may create through `thread create` without Role or through topic placement; either form creates the root without creating a Participant when no Role is stated. | Must | Participants §§2–3, Fable decisions 2026-09-14 |
...
| U7 | A session identity must have an open Participant before it posts a Thread reply or starts a Listen, and must be the open Orchestrator to resolve the Thread. A human identity may read, post, Listen, and resolve without a Participant; human resolution still closes every open Participant. | Must | Participants §2 invariant 3; Fable final review 2026-09-14 |
