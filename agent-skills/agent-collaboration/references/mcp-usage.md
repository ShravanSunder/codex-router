...
3. Resolve exact identities before mutation. A conversation target is the complete `SessionRef` with service ID, endpoint ID, and session ID. Preserve the current sender and use explicit agent attribution; never substitute a title, working directory, native-thread inventory entry, board root, or human identity.
