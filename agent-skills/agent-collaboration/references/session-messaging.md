...
A top-level conversation and a native child are session/runtime ancestry, not board seats or caller roles. A direct conversation with a session does not transfer the sender's role, assignment, design authority, filesystem access, or native-child direct-input capability; the caller's assignment remains the authority, and escalation returns to that caller when the assignment requires it. A board seat is discussion-local participation; use the [five-seat semantics](message-board.md#five-board-seats) reference for its limits.
...
Use actual resolved addresses and a content file. `--delivery auto` starts/resumes or steers as needed; `steer` requires active work and `queue` requires a loaded recipient. A queued message is not proof the recipient processed it. For an explicit reply, use the incoming Self-declared sender address as the recipient, while keeping your own sender identity, and send the requested answer or receipt to that original sender in the message content. Sending is not a reply unless it targets that exact sender; Router's own deliveries (Batch sets, heartbeats, finalizations from a session listen) arrive with the header `Router delivery` and an intended recipient, no self-declared sender; they carry no reply address and are not replied to. If a reply is needed, include the return address and requested answer or receipt.
...
For `--new`, set `--access write-restricted|workspace-write` as well as the required model and effort; `--fork` states `--access` and inherits the rest unless a change is deliberate. `write-restricted` permits the project's `tmp/` and `docs/wip/` plus shared scratch; `workspace-write` permits the selected worktree plus shared scratch. `--root-message-id` is optional and names the shared scratch scope by one board root message ID. It neither joins the session to that Thread nor links that root to another root; use board participation and ordinary message references for those separate purposes. `--approver` records the client approval authority; it defaults to the creating session. Resume passes the session and cwd only; model, effort, access, root association, and scratch path persist. Pass `--effort` on resume only to change it deliberately: a changed effort or model is a cache-affecting setting, the receipt reports `effortChange`, and whether the provider prompt cache is reused is provider- and route-dependent and requires evidence. Fork may omit `--model` and `--effort` to inherit the source thread's values. A model name, session name, root association, or board role does not establish assignment write permission or replace the session's exact `SessionRef`.
