...
Use caller-supplied timing, eligibility, recipient, and lifetime. This reference owns the wake operation, not caller scheduling or cache policy. A wake runs a real agent turn; neither saving it nor reaching its firing time proves cache reuse. A later follow-up uses the same recorded recipient address, not a newly created session. Cron expressions must match actual intended firing times; minute fields `0,45` alternate 45- and 15-minute gaps.
