...
- When you create or fork a conversation for an agent, give it the `manage-agents` title `<emoji> <role> · <purpose>` (for example `🐒 Sidekick · parser fix` or `🔎 Review Sidekick · PR 91`) through the supported rename or display route, and verify the saved visible title. If no route exists, report that capability gap. A title never replaces the SessionRef.
