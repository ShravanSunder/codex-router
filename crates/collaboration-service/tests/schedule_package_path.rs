...
async fn exercise_package(fresh: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if fresh {
        if !matches!(
            ...
            collaboration_protocol::ExecutionDestination::FreshEachRunUnprepared
        ) {
            return Err("SDK import lost fresh mode or retained local binding".into());
        }
        let enable = client
            .enable_schedule(collaboration_protocol::ScheduleEnableRequest {
                operation_id: OperationId::generate(),
                schedule_id: imported.schedule_id.clone(),
            })
            .await;
        if !matches!(enable, Err(collaboration_client::ScheduleClientError::Rejected(ref error)) if error.field.as_deref() == Some("destination") && error.message.contains("schedule update"))
        {
        ...
        imported = client
            .update_schedule(collaboration_protocol::ScheduleUpdateRequest {
                operation_id: OperationId::generate(),
                schedule_id: imported.schedule_id.clone(),
            ...
            .await?;
        if !matches!(imported.definition.destination, collaboration_protocol::ExecutionDestination::FreshEachRun { ref cwd, .. } if cwd == "/destination-workspace")
            || imported.definition.enabled
        {
    ...
    let oversized = client
        ...
        .await;
    match oversized {
        Err(collaboration_client::ScheduleClientError::Rejected(error))
            if matches!(error.details, collaboration_protocol::ScheduleFailureDetails::FrameLimit { encoded_bytes, maximum_bytes } if encoded_bytes > maximum_bytes)
                && error.field.as_deref() == Some("packageUtf8") => {}
        _ => return Err("oversized import did not return typed encoded-frame feedback".into()),
    }
    client
        .update_instruction(collaboration_protocol::InstructionUpdateParams {
            operation_id: OperationId::generate(),
    ...
    let oversized = client
        ...
        .await;
    match oversized {
        Err(collaboration_client::ScheduleClientError::Rejected(error)) if matches!(error.details, collaboration_protocol::ScheduleFailureDetails::FrameLimit { encoded_bytes, maximum_bytes } if encoded_bytes > maximum_bytes) =>
            {}
        _ => {
            return Err(
