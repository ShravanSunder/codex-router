...
mod tests {
    ...
    .unwrap_or_else(|error| panic!("scope: {error}"));
        ...
        let initial_position = collaboration_protocol::JournalPosition {
                ...
                if page.records.iter().any(|record| {
                    matches!(
                        ...
                        LifecycleChange::ThreadStatus {
                            status: collaboration_protocol::NativeThreadStatus::NotLoaded,
                            ordering: collaboration_protocol::ObservationOrdering::Established
                        }
                    )
                }) {
                    break;
        ...
        assert_eq!(
            ...
            Some(collaboration_protocol::NativeThreadStatus::Idle)
        );
        let changed = client
        ...
        assert!(changed.entries.iter().any(|entry| entry.last_status
            == Some(collaboration_protocol::NativeThreadStatus::NotLoaded)));
