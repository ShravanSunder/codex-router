...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let listed = client
        .list_wakeups(collaboration_protocol::AutomationPageRequest {
            cursor: None,
            limit: 50.try_into()?,
        })
        .await?;
    ...
    let pause_request = collaboration_protocol::WakeMutationRequest {
    ...
    if paused.wakeup.state != collaboration_protocol::WakeState::Paused {
        return Err("pause did not reach storage".into());
    }
    let resumed = client
        .resume_wakeup(collaboration_protocol::WakeMutationRequest {
            operation_id: OperationId::generate(),
            wakeup_id: first.definition.wakeup_id.clone(),
        })
        .await?;
    if resumed.wakeup.state != collaboration_protocol::WakeState::Active
        || resumed.wakeup.definition.anchor_at != first.definition.anchor_at
    {
        return Err("resume changed original anchor".into());
    }
    let cancelled = client
        .cancel_wakeup(collaboration_protocol::WakeMutationRequest {
            operation_id: OperationId::generate(),
            wakeup_id: first.definition.wakeup_id,
        })
        .await?;
    if cancelled.wakeup.state != collaboration_protocol::WakeState::Cancelled {
    ...
    let claimed = store.lock().await.claim_delivery::<collaboration_protocol::SessionRef,collaboration_protocol::MessageContent,collaboration_protocol::CodexGeneration>(&delivery, chrono::Utc::now().timestamp_millis()).await?.ok_or("fixture claim missing")?;
    let attempts = client
        .read_delivery_attempts(collaboration_protocol::DeliveryAttemptsRequest {
            delivery_id: delivery.clone(),
            cursor: None,
    ...
    store
        ...
        .await?;
    let cancellation_request = collaboration_protocol::WakeMutationRequest {
        operation_id: OperationId::generate(),
