...
use collaboration_protocol::{EndpointDescription, JournalStatus, UuidIdentity};
...
pub(crate) async fn dispatch_journal(
    ...
    let Ok(params) = serde_json::from_value::<collaboration_protocol::JournalReadParams>(params)
    else {
        return invalid(id);
    };
    if !(1..=100).contains(&params.page_size)
