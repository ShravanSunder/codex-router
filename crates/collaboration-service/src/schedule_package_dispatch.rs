...
pub(crate) async fn dispatch(request: ScheduleRequest<'_>) -> Value {
    ...
    if request.method == "schedule/export" {
        ...
        if required_bytes > MAX_CONTROL_FRAME_BYTES {
            return json!({"jsonrpc":"2.0","id":request.id,"error":{"code":-32050,"message":"Schedule package too large","data":collaboration_protocol::ScheduleFailure::package_frame_limit(None, required_bytes)}});
        }
        return response;
    }
