...
#[tokio::test]
async fn summary_completion_requires_exact_attempt_and_retains_source()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut effects = summary.effects;
    effects.target = Some("summary-thread".to_owned());
    effects.generation = Some("g1".to_owned());
    effects.submission = SubmissionEffect::Accepted;
    effects.native_turn_id = Some("summary-turn".into());
    ...
    store
        .record_summary_progress(automation_storage::SummaryProgress {
            ...
            effects,
            target: Some("summary-thread".to_owned()),
    ...
    let stale = store
        .complete_summary::<String, String>(automation_storage::SummaryCompletion {
            ...
            text: InstructionText::try_from("stale".to_owned())?,
            now_ms: 601000,
        })
        .await?;
    if stale {
        return Err("stale summary attempt replaced current output".into());
    }
    if !store
        .complete_summary::<String, String>(automation_storage::SummaryCompletion {
            run_id: run.clone(),
            ...
            native_turn_id: "summary-turn".into(),
            text: InstructionText::try_from("Verified completed task context".to_owned())?,
            now_ms: 602000,
        })
        .await?
    {
        return Err("current summary completion not saved".into());
    }
    let current = store
        .read_run::<String, String, String, String>(&run)
        .await?;
    if current.phase != agent_automation::RunPhase::Finished
        || current.summary_text.as_deref() != Some("Verified completed task context")
        || current.worker_outcome.is_none()
    {
        return Err("summary completion lost worker outcome or current text".into());
    }
    store.close().await?;
