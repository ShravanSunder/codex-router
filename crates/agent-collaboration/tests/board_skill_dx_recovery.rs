...
async fn luna_recovers_from_combined_board_argument_diagnostic() -> ProofResult<()> {
    ...
    write_private_json(
        ...
        &json!({
            ...
            "agentCollaborationExecutable":env!("CARGO_BIN_EXE_agent-collaboration"),
    ...
    let diagnostic = "The board command was passed as one argument. Pass each word as a separate argument, for example: agent-collaboration board project list --json";
    let task = format!(
...
async fn wait_for_terminal_turn(
    ...
    agent: &collaboration_client::protocol::SessionRef,
