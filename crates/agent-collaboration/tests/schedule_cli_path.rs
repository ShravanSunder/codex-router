...
async fn cli_creates_and_inspects_disabled_schedule() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let exported = tokio::process::Command::new(env!("CARGO_BIN_EXE_agent-collaboration"))
        .args([
            "schedule",
            "export",
            "--schedule-id",
    ...
    let events = tokio::process::Command::new(env!("CARGO_BIN_EXE_agent-collaboration"))
    ...
    let prepared = tokio::process::Command::new(env!("CARGO_BIN_EXE_agent-collaboration"))
        .env_remove("CODEX_ROUTER_DEBUG_APP_SERVER_SOCKET")
        .env_remove("CODEX_ROUTER_USE_HOME_DEFAULT")
        .args([
            "schedule",
