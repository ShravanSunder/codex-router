...
enum PreparedCollection {
    RunReconcile(collaboration_client::protocol::RunShowRequest),
    DeliveryReconcile(DeliveryShowRequest),
    OperationReconcile(collaboration_client::protocol::OperationShowRequest),
    OperationShow(collaboration_client::protocol::OperationShowRequest),
    RunSummaries(collaboration_client::protocol::RunSummariesRequest),
    DeliveryAttempts(collaboration_client::protocol::DeliveryAttemptsRequest),
    Events(collaboration_client::protocol::AutomationEventsRequest),
...
enum ReadCommandError {
    Inspection(Box<collaboration_client::protocol::AutomationInspectionFailure>),
    Wake(Box<collaboration_client::protocol::WakeFailure>),
    Client(ClientError),
}
...
fn prepare(command: CollectionCommand) -> Result<PreparedCollection, String> {
    match command {
        CollectionCommand::RunReconcile(id) => Ok(PreparedCollection::RunReconcile(
            collaboration_client::protocol::RunShowRequest {
        ...
        CollectionCommand::OperationReconcile(id) => Ok(PreparedCollection::OperationReconcile(
            collaboration_client::protocol::OperationShowRequest {
                operation_id: id
                    .try_into()
        ...
        CollectionCommand::RunSummaries(options) => {
            ...
            Ok(PreparedCollection::RunSummaries(
                collaboration_client::protocol::RunSummariesRequest {
                    run_id: options
                        .run_id
                        .try_into()
        ...
        CollectionCommand::DeliveryAttempts(options) => {
            ...
            Ok(PreparedCollection::DeliveryAttempts(
                collaboration_client::protocol::DeliveryAttemptsRequest {
                    delivery_id: options
                        .delivery_id
                        .try_into()
        ...
        CollectionCommand::Events(options) => Ok(PreparedCollection::Events(
            collaboration_client::protocol::AutomationEventsRequest {
                after: options.after,
                limit: options
                    .limit
                    .try_into()
