...
#[derive(Parser)]
#[command(
    name = "agent-collaboration wake",
    bin_name = "agent-collaboration wake"
)]
...
fn prepare(command: WakeCommand) -> Result<WakeInvocation, String> {
    match command {
        ...
        WakeCommand::List {
            ...
            request: PreparedWake::List(collaboration_client::protocol::AutomationPageRequest {
                cursor,
                limit: limit
                    .try_into()
...
fn encode_result<TResult: serde::Serialize>(
    ...
    serde_json::to_value(result).map_err(|_| {
        collaboration_client::ClientError::Protocol("cannot encode wake result").into()
    })
}
