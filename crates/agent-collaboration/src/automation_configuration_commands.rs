...
#[derive(Parser)]
#[command(
    name = "agent-collaboration automation",
    bin_name = "agent-collaboration automation"
)]
    ...
    let (record, code) = match result {
        ...
        Err(ConfigurationClientError::Rejected(error)) => {
            let uncertain = !matches!(
                error.file_state,
                collaboration_client::protocol::ConfigurationFileState::NotReplaced
            );
            (
...
fn encode<TResult: serde::Serialize>(result: TResult) -> Result<Value, ConfigurationClientError> {
    serde_json::to_value(result).map_err(|_| {
        collaboration_client::ClientError::Protocol("invalid configuration output").into()
    })
}
