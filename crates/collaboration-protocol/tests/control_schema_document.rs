...
fn bound_native_thread_uses_exact_offline_schema_and_closed_control_result() {
    ...
    let mut response = json!({"jsonrpc":"2.0","id":"inspect-1","result":{
        ...
        "generation":{"serviceEpoch":id,"generation":1},"effectiveAccess":"workspace-write","effectiveApprovalPolicy":"on-request","effectiveApprovalsReviewer":"auto_review","thread":{"id":"thread"}
    }});
