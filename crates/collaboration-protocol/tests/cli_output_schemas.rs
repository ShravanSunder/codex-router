...
fn cli_catalog_covers_closed_envelopes_and_pinned_acp_variants() {
    ...
    assert!(conversation.is_valid(&serde_json::json!({"kind":"promptResult","target":target,"effectiveModel":"gpt-5.6-sol","effectiveEffort":"medium","effectiveAccess":"workspace-write","effectiveApprovalPolicy":"on-request","effectiveApprovalsReviewer":"auto_review","idleSeconds":0,"result":{"stopReason":"cancelled"}})));
    assert!(!conversation.is_valid(&serde_json::json!({"kind":"promptResult","target":target,"effectiveModel":"gpt-5.6-sol","effectiveEffort":"medium","effectiveAccess":"workspace-write","effectiveApprovalPolicy":"on-request","effectiveApprovalsReviewer":"auto_review","idleSeconds":0,"result":{"stopReason":"interrupted"}})));
