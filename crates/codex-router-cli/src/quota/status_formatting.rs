...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) enum QuotaHumanGroup {
    Preferred,
    Available,
    Held,
    BlockedOrStale,
...
pub(super) fn quota_human_group(row: &QuotaStatusRow) -> QuotaHumanGroup {
...
pub(super) fn quota_state_text(row: &QuotaStatusRow) -> &'static str {
    if row.preferred_next {
        return "preferred";
    }
...
pub(super) fn quota_window_visual_summary(
...
pub(super) fn quota_account_list_window_summary(
    windows: &[DisplayQuotaWindow],
    window_seconds: u64,
    label: &'static str,
    now_unix_seconds: u64,
...
pub(super) fn active_clients_label(row: &QuotaStatusRow) -> String {
...
pub(super) fn reset_credits_account_list_label(reset_credits_available: Option<u32>) -> String {
...
pub(super) fn reason_summary(row: &QuotaStatusRow) -> String {
...
pub(super) fn format_window_cell(
    windows: &[DisplayQuotaWindow],
    window_seconds: u64,
    now_unix_seconds: u64,
    unicode: bool,
...
pub(super) fn quota_bar(percent: u32, unicode: bool) -> String {
...
pub(super) fn format_reset_credits(reset_credits_available: Option<u32>) -> String {
    reset_credits_available.map_or_else(
        || "-".to_owned(),
        |credits| {
            if credits == 1 {
...
pub(super) fn format_active_clients(active_clients: ActiveClientMirrorStatus) -> String {
...
pub(super) fn format_refresh_status(
...
pub(super) const fn quota_refresh_error_class_label(
...
pub(super) fn format_burn_cell(assessment: &BurnDownAccountAssessment) -> String {
    if matches!(
        assessment.quota_evidence_reason(),
        QuotaEvidenceReason::NeedsQuotaProbe
            | QuotaEvidenceReason::MissingExpectedWindow
...
pub(super) fn format_window_pace(
    windows: &[DisplayQuotaWindow],
    window_seconds: u64,
    label: &'static str,
    now_unix_seconds: u64,
...
pub(super) fn format_run_rate_estimate(
...
pub(super) fn format_burn_rate_basis_points_per_hour(
...
pub(super) const fn run_rate_confidence_label(confidence: QuotaRunRateConfidence) -> &'static str {
    match confidence {
...
pub(super) fn format_routing_cell(assessment: &BurnDownAccountAssessment) -> String {
...
pub(super) const fn routing_reason_is_preferred(reason: RoutingReason) -> bool {
    matches!(
        reason,
...
pub(super) fn format_routing_reason(reason: RoutingReason) -> &'static str {
    match reason {
...
pub(super) fn format_next_use(assessment: &BurnDownAccountAssessment) -> &'static str {
    format_next_use_from_routing_reason(assessment.routing_reason())
}

pub(super) fn format_next_use_from_routing_reason(reason: RoutingReason) -> &'static str {
    match reason {
        RoutingReason::PreferredWeeklyHealthier
        | RoutingReason::PreferredNearResetDrainable
        | RoutingReason::PreferredNearResetControlledDrain
...
pub(super) fn format_percent(value: u32) -> String {
    format!("{}%", value.min(100))
}

pub(super) fn format_relative_time(target_unix_seconds: u64, now_unix_seconds: u64) -> String {
...
pub(super) const fn quota_window_status_from_selector_status(
...
pub(super) fn quota_window_label(limit_window_seconds: u64) -> &'static str {
