...
- sparse samples: produce low-confidence slope with clear confidence flag
...
- stale samples: downweight or mark unavailable
- acceleration: prefer recent samples but preserve one-week context

The algorithm must expose both:
- current remaining quota
- projected exhaustion time under active load
...
Required model:
- active reservation per selected account and route band
- reservation lifecycle on WebSocket open, request start, response complete, error, close, and task cancellation
- per-account projected burn under current active connections
- release guarantees on drop/error
- observability for leaked/stale reservations
...
The sessions command reads Codex local state metadata from `state_5.sqlite` read-only.
...
The implementation must also provide a live-gated E2E path using a real logged-in Codex account. This live path is not CI-default because real quota state is external, mutable, and account-specific. It must print sanitized evidence:
- account label
- route band
- 5h remaining/reset
...
- SQLite integration tests cover history, retention, reset boundaries, and current snapshots
